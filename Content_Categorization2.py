...
def get_published_date_cnn(soup):
    """
    Extracts the publication date from the CNN article soup.
    Args:
        soup (BeautifulSoup): The BeautifulSoup object for the article page.
    Returns:
        str: The formatted publication date or a default message if not found.
    """
    try:
        date_element = soup.find('div', class_='headline_live-story__byline-sub-text')
        if date_element:
            timestamp = date_element.find('div', class_='timestamp')
            return timestamp.get_text(strip=True) if timestamp else "Publication date not available."
    except Exception as e:
        logging.error(f"Error fetching publication date for CNN: {str(e)}")
    return "Publication date not available."
        ...
        try:
            ...
            if url == "https://edition.cnn.com/":
                for article in soup.find_all('div', class_='container__headline'):
                    title_element = article.find('span', class_='cd__headline-text')  # Extract the title element
                    link_element = article.find('a')  # Extract the <a> tag for the link

                    if title_element and link_element:
                        title = title_element.text.strip()  # Extract the article title
                        article_url = link_element.get('href')  # Get the URL from the <a> tag
                        if article_url and not article_url.startswith('http'):
